 ...
 * RedisTemplate 提供了五种数据结构操作类型 hash / list / set / zset / value 方法命名格式为 数据操作类型 + 操作 如 hashPut
 * 指以hash结构(也就是map)向key添加键值对
 */
public interface RedisHelper<HK, T> {
	 ...
	 * @param timeUnit 时间单位
	 */
	void set(String key, T domain, long timeout, TimeUnit unit);	
